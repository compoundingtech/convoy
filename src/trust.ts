...
/** Mark `dir` (resolved absolute) trusted in `~/.claude.json` so Claude Code doesn't prompt on launch.
 *  Merges into any existing project entry and writes atomically (temp + rename) to shrink the window for
 *  clobbering a concurrent Claude write. Best-effort: returns false (never throws) if the config can't be
 *  read/written — a launch must not fail because the trust pre-write hiccuped. Returns true when the
 *  folder is trusted (written now, or already was). */
