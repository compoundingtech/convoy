...
## What it is
`convoy` is the single front door to a smalltalk agent network — the nomad-like
orchestrator that pulls the lower layers into one runnable whole. It composes
**@compoundingtech/smalltalk** (the message bus + agent status, driven by the
`st` CLI) and **@compoundingtech/pty** (persistent terminal sessions) into one
...
## The idiom (happy path — adopt it in isolation)
```sh
convoy init myproj                              # stand up a network at <home>/myproj (name → CoS)
convoy doctor                                   # prove the setup can do real agent work (honest — no false negatives)
